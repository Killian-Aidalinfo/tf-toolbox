...
# Forked repository https://github.com/angristan/wireguard-install
...
function installWireGuard() {
	...
	if [[ ${OS} == 'ubuntu' ]] || [[ ${OS} == 'debian' && ${VERSION_ID} -gt 10 ]]; then
        ...
        if [[ ${VERSION_ID} -eq 12 ]]; then
            # Pour Debian 12 Bookworm, ajoutez les backports si nécessaire
            if ! grep -rqs "^deb .* bookworm-backports" /etc/apt/; then
                echo "deb http://deb.debian.org/debian bookworm-backports main" >/etc/apt/sources.list.d/backports.list
                apt-get update
            fi
        elif [[ ${VERSION_ID} -lt 10 ]]; then
            # Gestion des versions antérieures de Debian (par exemple Buster)
            echo "Votre version de Debian (${VERSION_ID}) n'est pas prise en charge. Veuillez utiliser Debian 10 Buster ou une version ultérieure"
            exit 1
        fi
		apt update
